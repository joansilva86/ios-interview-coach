...
qa_bank.csv                   — question/answer bank for study and for sharing (e.g. with an English teacher). Schema: category,question,answer,on_point_date,asked_flag (on_point_date: date of the most recent On Point answer in practice, empty if none yet; asked_flag: set to 1 by the MCP's pickQuestion when it serves the question, cleared for all rows by clearSession — tracks what was already drilled this review cycle). category uses the exact strings from topic_catalog.csv. Answers are polished spoken-English model answers. Tracked. Structured writes go through the trainer-qa MCP.
