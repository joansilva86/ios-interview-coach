...
- **Canonical pattern labels**: `passive -ed` ("is use" → "is used") · `third-person -s` ("it fail" → "it fails") · `present perfect` ("I never faced" → "I've never faced") · `question mark in questions` · `word order: modifier before noun` ("framework Observation" → "Observation framework") · `comma splice` · `duplicated subject it` ("add a hook it is good" → "adding a hook is good") · `gerund subject` ("add a hook is" → "adding a hook is") · `article a/an` · `preposition choice` ("work in a branch" → "work on a branch")
